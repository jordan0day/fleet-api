defmodule FleetApi.Direct.Test do
  ...
  setup_all do
    ...
    FleetApi.Direct.set_node_url("http://localhost:7002")
    :ok
  end
  ...
  test "get_unit" do
    use_cassette "get_unit", custom: true do
      {:ok, unit} = get_unit("subgun-http.service")
  ...
  test "delete_unit" do
    use_cassette "delete_unit", custom: true do
      assert :ok = delete_unit("subgun-http.service")
    end
  end

  test "list_machines" do
    use_cassette "list_machines", custom: true do
      {:ok, machines} = list_machines()
  ...
  test "create unit" do
    use_cassette "set_unit_new", custom: true do
      ...
      assert :ok = set_unit("test.service", unit)
    end
  end

  test "update_unit_desired_state" do
    use_cassette "set_unit_update", custom: true do
      assert :ok = set_unit("test.service", "launched")
    end
  end
end